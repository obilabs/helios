-- 095_login_activity.sql
--
-- Revives archived migration 066 verbatim. The Login Locations dashboard widget reads
-- login_activity and google-workspace.service writes to it, but the table was only ever
-- defined in database/archived_migrations/ and never applied. The widget's routes also
-- read the organization from the wrong request property, so every call from the
-- signed-in UI failed with 401 before it could reach the missing table. The 401 masked
-- the missing table; fixing one exposed the other (2026-09-11).
--
-- Idempotent: CREATE ... IF NOT EXISTS / CREATE OR REPLACE throughout, so an install
-- that somehow already has it is untouched.
