...
describe('Org Chart Routes', () => {
  ...
  describe('GET /api/org-chart', () => {
    it('should return org chart with hierarchy structure', async () => {
      ...
      mockQuery.mockResolvedValueOnce({
        rows: [
          { user_id: '1', first_name: 'Jack', last_name: 'Chen', email: 'jack@obilabs.dev', job_title: 'CEO', department: 'Executive', level: 0, reporting_manager_id: null },
          { user_id: '2', first_name: 'Sarah', last_name: 'Chen', email: 'sarah@obilabs.dev', job_title: 'CTO', department: 'Engineering', level: 1, reporting_manager_id: '1' },
          { user_id: '3', first_name: 'Michael', last_name: 'Rodriguez', email: 'michael@obilabs.dev', job_title: 'VP Engineering', department: 'Engineering', level: 2, reporting_manager_id: '2' },
        ],
      });
    ...
    it('should return orphaned users separately', async () => {
      ...
      mockQuery.mockResolvedValueOnce({
        rows: [
          { user_id: '101', first_name: 'Frank', last_name: 'Thompson', email: 'frank@obilabs.dev', job_title: 'Security Consultant', department: 'IT', reporting_manager_id: 'deleted-user-id' },
        ],
      });
    ...
    it('should build correct tree structure with direct reports', async () => {
      ...
      mockQuery.mockResolvedValueOnce({
        rows: [
          { user_id: 'ceo', first_name: 'John', last_name: 'CEO', email: 'ceo@obilabs.dev', job_title: 'CEO', department: 'Executive', level: 0, reporting_manager_id: null },
          { user_id: 'cto', first_name: 'Jane', last_name: 'CTO', email: 'cto@obilabs.dev', job_title: 'CTO', department: 'Engineering', level: 1, reporting_manager_id: 'ceo' },
          { user_id: 'dev1', first_name: 'Dev', last_name: 'One', email: 'dev1@obilabs.dev', job_title: 'Developer', department: 'Engineering', level: 2, reporting_manager_id: 'cto' },
        ],
      });
  ...
  describe('PUT /api/users/:userId/manager', () => {
    it('should update manager relationship', async () => {
      ...
      // Mock update query
      mockQuery.mockResolvedValueOnce({
        rows: [{ id: 'user-1', email: 'user@obilabs.dev', first_name: 'Test', last_name: 'User', reporting_manager_id: 'manager-1' }],
      });
    ...
    it('should allow removing manager (setting to null)', async () => {
      ...
      // Mock update query
      mockQuery.mockResolvedValueOnce({
        rows: [{ id: 'user-1', email: 'user@obilabs.dev', first_name: 'Test', last_name: 'User', reporting_manager_id: null }],
      });
  ...
  describe('GET /api/users/:userId/direct-reports', () => {
    it('should return direct reports for a manager', async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [
          { user_id: 'report-1', email: 'report1@obilabs.dev', first_name: 'Report', last_name: 'One', job_title: 'Developer', department: 'Engineering', direct_reports_count: '0' },
          { user_id: 'report-2', email: 'report2@obilabs.dev', first_name: 'Report', last_name: 'Two', job_title: 'Developer', department: 'Engineering', direct_reports_count: '2' },
        ],
      });
