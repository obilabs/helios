...
test.describe('Better Auth Integration Diagnosis', () => {
  ...
  test.describe('5. Database State', () => {
    ...
    test('Log database check commands', async () => {
      ...
      console.log('docker exec helios-db psql -U postgres -d helios -c "SELECT id, name, domain FROM organizations;"')

      console.log('\n# Check test user exists:')
      console.log(`docker exec helios-db psql -U postgres -d helios -c "SELECT id, email, role, is_active, password_hash IS NOT NULL as has_password FROM organization_users WHERE email='${TEST_CONFIG.testEmail}';"`)

      console.log('\n# Check auth tables exist:')
      console.log('docker exec helios-db psql -U postgres -d helios -c "\\dt auth_*"')

      console.log('\n# Check for any sessions:')
      console.log('docker exec helios-db psql -U postgres -d helios -c "SELECT id, user_id, expires_at FROM auth_sessions ORDER BY created_at DESC LIMIT 5;"')
