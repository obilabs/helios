...
6. **Check backend logs:**
   ```bash
   docker logs helios_backend --tail=50 | grep "invalid_scope"
   ```

   You'll see:
   ```
   [error]: Google SDK request failed {
     "path": "admin/directory/v1/users/mike@obilabs.dev",
     "method": "GET",
...
7. **Prove setup is correct by running direct test:**
   ```bash
   docker exec helios_backend node /app/test-gw-api-direct.js
   ```
...
- **Google Workspace:** obilabs.dev domain

### Check Installed Versions

```bash
docker exec helios_backend npm list googleapis google-auth-library
```
