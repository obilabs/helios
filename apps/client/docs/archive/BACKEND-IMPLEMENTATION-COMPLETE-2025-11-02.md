...
curl -X POST http://localhost:3001/api/organization/users/{userId}/block \
  ...
  -d '{
    ...
    "delegateTo": "security@obilabs.dev",
    "emailForwarding": {
      "enabled": true,
      "forwardTo": ["security@obilabs.dev", "hr@obilabs.dev"]
    }
  }'
...
- Emails to user@obilabs.dev forward to security@ and hr@
```
...
1. Get JWT token (login as testproxy@obilabs.dev)
