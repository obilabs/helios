...
Implement an opt-in telemetry service that sends anonymous usage data to helios.obilabs.dev. This enables usage-driven development where popular API relay calls become UI features and frequent commands become buttons/shortcuts.
...
- Publishes usage insights publicly at helios.obilabs.dev/insights
...
class TelemetryService {
  ...
  // Send heartbeat to helios.obilabs.dev
  async sendHeartbeat() {
    if (!this.enabled) return;
    ...
    await fetch('https://helios.obilabs.dev/api/instances/heartbeat', {
      method: 'POST',
