...
docker exec helios_client_postgres psql -U postgres -d helios_client -c "UPDATE organization_users SET is_active = true, status = 'active', user_status = 'active' WHERE email = 'jack@obilabs.dev';"
...
docker exec helios_client_postgres psql -U postgres -d helios_client -c "SELECT email, is_active, status FROM organization_users WHERE email = 'jack@obilabs.dev';"
```
