...
const TEST_EMAIL = 'jack@obilabs.dev';
const TEST_PASSWORD = '***REMOVED-CREDENTIAL***';

const TEST_USER_EMAIL = 'playwrighttest@obilabs.dev';
const TEST_GROUP_EMAIL = 'playwrighttestgroup@obilabs.dev';
...
test('Manual CRUD Test - User Operations', async ({ page }) => {
  ...
  expect(output).toContain('jack@obilabs.dev');
  ...
  expect(output.toLowerCase()).toContain('obilabs.dev');
  console.log('✅ helios gw users list - PASSED');

  console.log('\n=== TEST 3: helios gw users get (existing user) ===');
  output = await runCommand(page, 'helios gw users get mike@obilabs.dev');
  ...
  if (output.toLowerCase().includes('error') || output.includes('invalid_scope')) {
    ...
    expect(output).toContain('mike@obilabs.dev');
  ...
  console.log('\n=== TEST 6: CREATE USER ===');
  output = await runCommand(page, `helios gw users create ${TEST_USER_EMAIL} --firstName=Playwright --lastName=Test --password=***REMOVED-CREDENTIAL***`);
