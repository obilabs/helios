...
**Test domain:** `obilabs.dev`
**Test admin:** `mike@obilabs.dev`
...
const auth = new google.auth.JWT({
  ...
  subject: 'mike@obilabs.dev'
});

(async () => {
  await auth.authorize();
  const admin = google.admin({ version: 'directory_v1', auth });
  const res = await admin.users.list({ domain: 'obilabs.dev', maxResults: 5 });
  console.log('Users:', res.data.users.map(u => u.primaryEmail));
})();
"
```

Expected output: List of users including mike@obilabs.dev
