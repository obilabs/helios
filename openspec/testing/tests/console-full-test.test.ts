 ...
 *
 * Test User: jack@obilabs.dev / ***REMOVED-CREDENTIAL***
 */
...
const TEST_USER_EMAIL = 'testuser@obilabs.dev';
const TEST_USER_FIRST = 'Test';
const TEST_USER_LAST = 'User';
const TEST_USER_PASSWORD = '***REMOVED-CREDENTIAL***';

// Test group to create and delete
const TEST_GROUP_EMAIL = 'testgroup@obilabs.dev';
...
test.describe('Developer Console - Full CRUD Testing', () => {
  ...
  test('2. helios gw users list - should show Google Workspace users', async ({ page }) => {
    ...
    expect(output).toMatch(/anthony@obilabs\.io|mike@obilabs\.io|pewter@obilabs\.io/);
  });

  test('3. helios gw users get - should fetch single user details', async ({ page }) => {
    await executeCommand(page, 'helios gw users get mike@obilabs.dev');
    await waitForCommandComplete(page);
