...
import { generateStrongPassword, generatePin } from '../lib/password-generator';
...
export function DeveloperConsole({ organizationId, isPopup = false }: DeveloperConsoleProps) {
  ...
  const handleCreateUser = async (args: string[]) => {
    if (args.length < 1) {
      ...
      addOutput('info', '  --password=auto     Auto-generate a strong random password');
  ...
  const handlePasswordCommand = (args: string[]) => {
    ...
    switch (action) {
      ...
      case 'gen': {
        ...
        const style = params.style || 'random';
        ...
        for (let i = 0; i < Math.min(count, 20); i++) {
          ...
          switch (style) {
            ...
            case 'random':
            default:
              pwd = generateStrongPassword(parseInt(params.length || '16', 10));
              break;
          }
          passwords.push(pwd);
      ...
      case 'styles': {
        ...
        addOutput('info', '='.repeat(50));
        addOutput('info', '  random      16+ random characters, upper/lower/digit/symbol (default)');
        addOutput('info', `              Example: ${generateStrongPassword()}`);
        addOutput('info', '');
      ...
      default:
        addOutput('error', 'Usage: helios password generate [--count=N] [--style=random|pin] [--length=N]');
    ...
    switch (action) {
      ...
      case 'create': {
        ...
        if (args.length < 1) {
          ...
          addOutput('info', '       Use --password=auto to generate a strong random password');
          return;
        }
        const email = args[0];
        const params = parseArgs(args.slice(1));
      ...
      case 'reset-password': {
        ...
        const generatePassword = () => generateStrongPassword();
              ...
              <div className="help-section">
                ...
                <table className="command-table">
                  <tbody>
                    ...
                    <tr>
                      <td className="command-name">password generate [--style=random|pin]</td>
                ...
                <table className="command-table">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className="command-desc">Create user in Google Workspace AND Helios. --password=auto generates a strong random password</td>
                    </tr>
                    <tr>
          ...
          <div className="modal-content glass-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="example-section">
                ...
                <code className="example-code">api POST /api/google/admin/directory/v1/users '&#123;"primaryEmail":"new.user@company.com","name":&#123;"givenName":"New","familyName":"User"&#125;,"password":"***REMOVED-CREDENTIAL***"&#125;'</code>
              </div>
