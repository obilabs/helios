...
Write-Host '   docker exec helios_postgres psql -U postgres -d helios -c "SELECT action, actor_email, created_at FROM activity_logs WHERE action LIKE '"'"'google_api_%'"'"' ORDER BY created_at DESC LIMIT 5;"' -ForegroundColor Cyan
Write-Host ""
Write-Host "3. Check synced users:" -ForegroundColor White
Write-Host '   docker exec helios_postgres psql -U postgres -d helios -c "SELECT email, google_workspace_id, platforms FROM organization_users WHERE google_workspace_id IS NOT NULL LIMIT 5;"' -ForegroundColor Cyan
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
