...
docker exec -i helios_postgres psql -U postgres -d helios << 'EOF'
...
for MIGRATION_FILE in "$SCRIPT_DIR"/migrations/*.sql; do
    if [ -f "$MIGRATION_FILE" ]; then
        ...
        APPLIED=$(docker exec helios_postgres psql -U postgres -d helios -t -c \
        ...
        if [ "$APPLIED" = "1" ]; then
            ...
            if docker exec -i helios_postgres psql -U postgres -d helios < "$MIGRATION_FILE" > /dev/null 2>&1; then
                # Record migration
                docker exec helios_postgres psql -U postgres -d helios -c \
