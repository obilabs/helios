...
docker exec helios_postgres psql -U postgres -d helios -f /path/to/schema_organization.sql
```
...
```bash
# Full schema export
docker exec helios_postgres pg_dump -U postgres -d helios --schema-only > export.sql

# List all tables
docker exec helios_postgres psql -U postgres -d helios -c "\dt"

# Describe a specific table
docker exec helios_postgres psql -U postgres -d helios -c "\d organization_users"
```
...
docker exec helios_postgres pg_dump -U postgres -d helios --schema-only --no-owner --no-privileges > schema_organization.sql
```
...
Connection string:
```
postgresql://postgres:postgres@localhost:5432/helios
```
...
**Development:**
```bash
docker exec helios_postgres pg_dump -U postgres -d helios > backup.sql
```
