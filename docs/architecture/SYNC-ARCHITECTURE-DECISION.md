...
1. **Groups not synced to gw_group_members table**
   ```sql
   SELECT * FROM gw_group_members WHERE member_email = 'anthony@obilabs.dev';
...
docker-compose exec -T postgres psql -U postgres -d helios -c "SELECT email FROM organization_users WHERE first_name LIKE 'Anthony%';"

# Check if groups are synced
docker-compose exec -T postgres psql -U postgres -d helios -c "SELECT COUNT(*) FROM gw_group_members;"

# Check if group sync populated data
docker-compose exec -T postgres psql -U postgres -d helios -c "SELECT COUNT(*) FROM gw_groups;"
```
