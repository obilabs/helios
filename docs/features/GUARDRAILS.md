...
# helios_backend    Up (healthy)
# helios_frontend   Up (healthy)
# helios_postgres   Up (healthy)
# helios_redis      Up (healthy)
```
...
- Database is populated with test data (jack@obilabs.dev)
...
- [ ] Test account can log in (jack@obilabs.dev)
...
docker exec helios_postgres psql -U postgres -d helios -c "SELECT id, name FROM organizations;"

# Should show: ObiLabs organization
...
docker-compose ps
docker inspect helios_frontend | findstr "Mounts"
```
...
docker exec helios_postgres psql -U postgres -d helios -c "SELECT email FROM organization_users WHERE email='jack@obilabs.dev';"
...
- Test account can log in (jack@obilabs.dev / ***REMOVED-CREDENTIAL***)
