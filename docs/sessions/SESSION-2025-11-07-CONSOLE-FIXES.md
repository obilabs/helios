...
- Can list users
- Found 4 users: anthony@obilabs.dev, coriander@obilabs.dev, mike@obilabs.dev, pewter@obilabs.dev
...
1. **Verify backend loaded new transparent-proxy code:**
   ```bash
   docker exec helios_backend cat /app/src/middleware/transparent-proxy.ts | grep "await jwtClient.authorize"
   ```
