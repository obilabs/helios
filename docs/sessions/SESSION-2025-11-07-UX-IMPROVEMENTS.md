...
$ helios gw users get mike@obilabs.dev --format=json

# Also works for groups
$ helios gw groups get all-staff@obilabs.dev
$ helios gw groups get all-staff@obilabs.dev --format=json
```
...
===========================================================================
aberdeen@obilabs.dev           aberdeen                        active
anthony@obilabs.dev            anthony                         active
chikezie@obilabs.dev           chikezie                        active
```
...
EMAIL                         FIRST NAME     LAST NAME      STATUS
===========================================================================
aberdeen@obilabs.dev           Aberdeen        Ingrid          deleted
anthony@obilabs.dev            Anthony         Chike           suspended
chikezie@obilabs.dev           Chikezie        Aham            deleted
jack@obilabs.dev                Jack            Dribber         staged
mike@obilabs.dev               Michael         Agu             active
pewter@obilabs.dev             Pewter          Nwadi           suspended
```
...
helios gw users create newuser@obilabs.dev --firstName=John --lastName=Doe --password=Pass123!

# Delete user
helios gw users delete newuser@obilabs.dev
```
...
# Create group
helios gw groups create newgroup@obilabs.dev --name="New Group" --description="Test group"

# Delete group
helios gw groups delete newgroup@obilabs.dev
```
...
```bash
$ $user = helios gw users get mike@obilabs.dev
Unknown command: $user = helios gw users get mike@obilabs.dev
```
...
helios gw users get mike@obilabs.dev --format=json > user.json

# Or use jq for filtering
helios gw users get mike@obilabs.dev --format=json | jq '.primaryEmail'

# Or use JavaScript in browser console
const user = await fetch('/api/google/admin/directory/v1/users/mike@obilabs.dev', {
...
# User: jack@obilabs.dev, Password: ***REMOVED-CREDENTIAL***
...
3. **Custom Field Selection**
   ```bash
   helios gw users get mike@obilabs.dev --fields email,name,status
   ```

4. **Variable Support (Advanced)**
   ```bash
   set $user = helios gw users get mike@obilabs.dev
   echo $user.email
   ```
